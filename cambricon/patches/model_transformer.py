...
def get_norm(config):
    ...
    if args.normalization == "LayerNorm" and args.apply_layernorm_rms == False:
        return LayerNorm(config.hidden_size, eps=config.layernorm_epsilon)
...
def ParallelTransformerLayerInit(
    self,
    config,
    layer_number,
...
def ParallelAttentionForward(
    ...
    if (
        ...
        if not args.bf16:
            ...
            key_layer = _repeat_interleave_op(
                key_layer,
                self.num_attention_heads_per_partition
                // self.num_query_groups_per_partition,
            )
            value_layer = _repeat_interleave_op(
                value_layer,
                self.num_attention_heads_per_partition
                // self.num_query_groups_per_partition,
            )
...
megatron.model.transformer.ParallelTransformerLayer.__init__ = (
    ParallelTransformerLayerInit
)
