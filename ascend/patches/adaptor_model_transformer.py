...
def ParallelAttentionInit(
        ...
        if args.mup is None:
            ...
            if args.apply_init_customized:
                with tensor_parallel.get_cuda_rng_tracker().fork():
                    ...
                    wq = self.query_key_value.weight.view(new_tensor_shape)[
                        :, 0:tmp:, :
                    ]
                    wk = self.query_key_value.weight.view(new_tensor_shape)[
                        :, tmp : tmp + 1 :, :
                    ]
                    wv = self.query_key_value.weight.view(new_tensor_shape)[
                        :, tmp + 1 : tmp + 2, :, :
                    ]
    ...
    if self.use_flash_attn:
        self.core_attention_flash = FlashSelfAttention(
            ...
            pre_tokens=args.npu_fa_pre_tokens,
            next_tokens=args.npu_fa_next_tokens,
...
def ParallelAttentionForward(
    ...
    if rotary_pos_emb is not None:
        ...
        if self.rotary_interleaved_patch:
            ...
            query_layer = rearrange(
                query_layer, "... (two headdim) -> ... two headdim", two=2
            )
            query_layer = rearrange(
                query_layer.transpose(-2, -1),
                "... headdim two -> ... (headdim two)",
                two=2,
            )
            key_layer = rearrange(
                key_layer, "... (two headdim) -> ... two headdim", two=2
            )
            key_layer = rearrange(
            ...
            value_layer = rearrange(
                value_layer, "... (two headdim) -> ... two headdim", two=2
            )
            value_layer = rearrange(
                value_layer.transpose(-2, -1),
                "... headdim two -> ... (headdim two)",
                two=2,
            )
    ...
    if not self.use_flash_attn:
        ...
        if self.shape_order == "BSH":
            q, k, v = [
                rearrange(x, "s b h d -> b s (h d)").contiguous()
                for x in (query_layer, key_layer, value_layer)
            ]
        elif self.shape_order == "SBH":
            q, k, v = [
                rearrange(x, "s b h d -> s b (h d)").contiguous()
                for x in (query_layer, key_layer, value_layer)
            ]
        elif self.shape_order == "BNSD":
            q, k, v = [
                rearrange(x, "s b h d -> b h s d").contiguous()
                for x in (query_layer, key_layer, value_layer)
            ]
            ...
            raise ImportError(
                "flash attention shape order must be SBH or BSH, please add args shape-order"
            )

        if not self.sequence_parallel:
            with tensor_parallel.get_cuda_rng_tracker().fork():
                context_layer = self.core_attention_flash(
                    q, k, v, hidden_head_num, attention_mask
                )
        else:
            context_layer = self.core_attention_flash(
                q, k, v, hidden_head_num, attention_mask
        ...
        if self.shape_order == "BSH":
            context_layer = (
                torch.tensor(1.0).to(context_layer.dtype).npu() * context_layer
            )
...
def CoreAttentionForward(self, query_layer, key_layer, value_layer, attention_mask):
