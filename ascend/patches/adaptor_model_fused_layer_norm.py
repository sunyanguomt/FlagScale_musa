...
def MixedFusedLayerNormInit(
    ...
    assert not (
        self.apply_layernorm_1p and self.apply_layernorm_rms
    ), "Cannot apply both 1p and rms layernorm"

    self.init_weight = init_weight
    assert self.init_weight is None or isinstance(
        self.init_weight, float
    ), "Cannot init_weight of None or of non-float"
    assert not (
        self.init_weight is not None and self.apply_layernorm_1p
