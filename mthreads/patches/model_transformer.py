    ...
    def __init__(
        ...
        if self.use_flash_attn:
            if args.mup != "apply":
                ...
                softmax_scale = args.mup_attn_multiplier / float(
                    self.hidden_size_per_attention_head
                )
                self.core_attention_flash = FlashSelfAttention(
                    causal=True,
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(
        ...
        self.num_layers = _get_num_layers(
            args, ModelType.encoder_or_decoder, layer_type == LayerType.decoder
        )
        ...
        if torch.distributed.get_rank() % 8 == 0:
            print(
                ...
                config.recompute_granularity,
                "config.recompute-method",
                config.recompute_method,
                "self.pp_layer_num > self.recompute_num_layers",
                self.pp_layer_num > self.recompute_num_layers,
        ...
        if args.apply_init_norm_customized:
            self.init_weight_attn_norm = args.init_weight_attn_norm[
                self.layer_number - 1
            ]
    ...
    def _checkpointed_layer_norm(self, hidden_states, layrnorm_func):
        ...
        hidden_states = tensor_parallel.checkpoint(
            custom_forward, False, hidden_states, layrnorm_func
        )

        return hidden_states

    def _checkpointed_attention(
        self, query_layer, key_layer, value_layer, attention_mask, rotary_pos_emb=None
    ):
        """Forward method with activation checkpointing."""
    ...
    def checkpoint_attention_layernorm(
        self,
        hidden_states,
        attention_mask,
        encoder_output=None,
    ...
    def forward(
        ...
        if (
            self.recompute_num_layers != 1
            and self.pp_layer_num > self.recompute_num_layers
        ):
            layernorm_input, layernorm_output = self.checkpoint_attention_layernorm(
                hidden_states,
                attention_mask,
                encoder_output,
                enc_dec_attn_mask,
