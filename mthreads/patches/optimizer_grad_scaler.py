...
class MegatronGradScaler(ABC):
    ...
    def __init__(self, initial_scale):
        ...
        self._scale = torch.musa.FloatTensor([initial_scale])
...
class DynamicGradScaler(MegatronGradScaler):
    ...
    def __init__(
        ...
        self.min_scale = torch.musa.FloatTensor([min_scale])
        # Growth and backoff factors for the scale.
        assert growth_factor > 1.0
        self.growth_factor = torch.musa.FloatTensor([growth_factor])
        assert backoff_factor < 1.0
        assert backoff_factor > 0.0
        self.backoff_factor = torch.musa.FloatTensor([backoff_factor])
