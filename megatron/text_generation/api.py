...
def generate_and_post_process(model,
    ...
    # Main inference.
        ...
        # Only post-process on first stage.
        if mpu.is_pipeline_first_stage():
            print(f"device is {torch.musa.current_device()}, stream function return is {tokens}")
            return tokens
...
def beam_search_and_post_process(model,
    ...
    # Only post-process on first stage.
    if mpu.is_pipeline_first_stage():
        lengths = tokens.size(1)*torch.ones(beam_size, dtype=torch.int64, device='musa:{}'.format(torch.musa.current_device())) 
