...
def generate_tokens_probs_and_return_on_first_stage_stream_sub_process(
    ...
    if mpu.is_pipeline_last_stage():
        ...
        generated_sequence_lengths = torch.ones(
                ...
                device='musa:{}'.format(torch.musa.current_device())) * max_sequence_length
...
def beam_search_and_return_on_first_stage(model, tokens, lengths, beam_size, stop_token, num_return_gen, length_penalty, prevent_newline_after_colon=True):
    ...
    best_batches = None
    done = torch.zeros(1, dtype=torch.uint8, device='musa:{}'.format(torch.musa.current_device()))
    scores = torch.zeros(beam_size,
                         dtype=torch.float32,
                         device='musa:{}'.format(torch.musa.current_device())).unsqueeze(1)
    ...
    with torch.no_grad():
        ...
        for context_length in range(prompt_length, final_sequence_length):
            ...
            if mpu.is_pipeline_last_stage():
                ...
                if beam_hyp.is_done(best_scores.max().item(), context_length + 1 - prompt_length):
                    done = torch.ones(1, dtype=torch.uint8, device='musa:{}'.format(torch.musa.current_device()))
        ...
        if mpu.is_pipeline_last_stage():
            ...
            scores_size_tensor = torch.tensor(scores.shape, dtype=torch.int64, device='musa:{}'.format(torch.musa.current_device()))
            tokens_size_tensor = torch.tensor(tokens.shape, dtype=torch.int64, device='musa:{}'.format(torch.musa.current_device()))
