...
from megatron.text_generation.api_single_thread import (
    generate_and_post_process_single_thread,
)
import re
import shutil
import time
...
if __name__ == "__main__":
    ...
    if torch_xmlir:
        ...
        os.sched_setaffinity(
            os.getpid(), [xpu_to_cpu_mapping[torch_xmlir.xpu.current_device()]]
        )
    ...
    if torch_xmlir:
        ...
        gloo_group = torch.distributed.new_group(
            backend="gloo", timeout=timedelta(days=365)
        )
    else:
        gloo_group = None
