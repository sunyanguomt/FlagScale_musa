...
def make_sft_prompts(prompts, template):
    ...
    for p in prompts:
        p = covert_prompt_to_input_ids_with_history(
            p, [], tokenizer, max_token=4096, template=template
        )
...
def predict(model, prompts, seed, max_length, topk, topp, t, sft, template):
    ...
    input_length = max(
        [len(tokenizer.tokenize(prompts[j])) for j in range(len(prompts))]
    )
    ...
    with lock:  # Need to get lock to keep multiple threads from hitting code
        try:
            ...
            response, _, response_logprobs, _ = generate_and_post_process(
                model,
                prompts=prompts,
...
def stop_signal(model_info, request_model_name, engine, prompt, server_port):
    print(
        f"model info is {model_info}, request model name is {request_model_name}, engine is {engine}, prompt is {prompt} "
    )
    global flag_rc, flag_hm, previous_data, unchanged_count

    if (
        model_info == request_model_name
        and engine == "####Subjective_Inference_Ending####"
    ):
        flag_hm = True
...
class UvicornServer:
    ...
    def init_flask(self):
        ...
        async def get_generate_h(request: Request):
            ...
            stop_signal(
                self.model_info,
                request_model_name=config.get("model_name", ""),
            ...
            s = time.time()
            res, input_length = await sync_to_async(predict)(
                self.model,
                contexts,
                seed,
        ...
        async def get_generate_h(request: Request):
            ...
            if sft:
                prompts = covert_prompt_to_input_ids_with_history(
                    prompts, history, tokenizer, 2048, template
                )
...
if __name__ == "__main__":
    ...
    if mpu.is_pipeline_first_stage() and mpu.get_tensor_model_parallel_rank() == 0:
        server_ins = UvicornServer(
            model, server_port=int(args.server_port), model_info=args.model_info
        )
        server_ins.run()
