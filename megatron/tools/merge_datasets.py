...
def main(args):
    ...
    for prefix in sorted(prefixes):
        ...
        cnt += 1
        if cnt % 50 == 0:
            print(f"{cnt} merge has cost {(time.time() - start_epoch) / 60} minutes")
            start_epoch = time.time()

    print(f"merge_file_ has cost {(time.time() - start) / 60:.3f} minutes")
    start = time.time()
    builder.finalize(args.output_prefix + ".idx")
    print(f"finalize has cost {(time.time() - start) / 60:.3f} minutes")
