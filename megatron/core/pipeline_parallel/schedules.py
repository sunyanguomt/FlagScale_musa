...
def forward_step(
    ...
    if parallel_state.is_pipeline_last_stage():
        if not collect_non_loss_data:
            ...
            # print('yehua', loss, num_microbatches, loss.size(), flush=True)
...
def recv_forward(tensor_shapes, config):
    ...
    for tensor_shape in tensor_shapes:
        if tensor_shape is None:
            #logger.info("shape none")
            input_tensors.append(None)
        else:
            #logger.info("shape not none")
            input_tensors.append(p2p_communication.recv_forward(tensor_shape, config))
            #logger.info("shape not none finish")
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    if not forward_only:
        ...
        output_tensors = []
    forward_data_store = []
    #logger.info(f'num_warmup_microbatches: {num_warmup_microbatches}, num_microbatches_remaining {num_microbatches_remaining}, num_microbatches {num_microbatches}')
    # Run warmup forward passes.
    ...
    for i in range(num_microbatches_remaining):
        ...
        #logger.info("forward_step finish")
        ...
        if forward_only:
            ...
            #logger.info(f"{i}, send_forward_recv_backward start")
            output_tensor_grad = send_forward_recv_backward(
                output_tensor, send_tensor_shapes, config
            )
            #logger.info("send_forward_recv_backward finish")
