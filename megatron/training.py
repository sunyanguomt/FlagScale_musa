...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True):
    ...
    parameter_number = sum([sum([p.nelement() for p in model_module.parameters()])
                for model_module in model])
    parameter_number = torch.LongTensor([parameter_number]).to('musa')
    torch.distributed.all_reduce(parameter_number, op=torch.distributed.ReduceOp.SUM, group=mpu.get_model_parallel_group())
    args.parameter_number = parameter_number.item()
    if mpu.get_data_parallel_rank() == 0:
        if torch.distributed.get_rank() == 0:
            print(f' > total parameter number is {parameter_number.item()}')
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    for key in loss_dict:
        if not skipped_iter:
            total_loss_dict[key] = total_loss_dict.get(
                key, torch.musa.FloatTensor([0.0])) + loss_dict[key]
        else:
    ...
    if iteration % args.log_interval == 0:
        ...
        whole_tps = batch_size * args.seq_length / elapsed_time_per_iteration
        chip_tps = whole_tps / args.world_size
        log_string += ' System tokens per second: {:.3E} |'.format(whole_tps)
        log_string += ' Tokens/p/sec: {:.3E} |'.format(chip_tps)
        theoryflops = 98e12
        log_string += ' MFU: {:.3E} |'.format(chip_tps * args.parameter_number * 6 / theoryflops)
...
def build_train_valid_test_data_loaders(
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        flags = torch.musa.LongTensor(
            [int(do_train), int(do_valid), int(do_test)])
    else:
        flags = torch.musa.LongTensor([0, 0, 0])
