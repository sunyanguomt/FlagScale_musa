...
def reverse_checkpoint(args):
    ...
    model = LlamaForCausalLM.from_pretrained(
        input_base_path, low_cpu_mem_usage=True, torch_dtype=params_dtype
    )

    print(f"Load ref model at {ref_model_path}")
    ref = torch.load(
        os.path.join(ref_model_path, "model_optim_rng.pt"), map_location="cpu"
    )
    ...
    for layer_i in range(n_layers):
        Wqkv = reverse_qkv(
            model.model.layers[layer_i].self_attn.q_proj.weight,
            model.model.layers[layer_i].self_attn.k_proj.weight,
        ...
        fc1 = reverse_fc1(
            model.model.layers[layer_i].mlp.gate_proj.weight,
            model.model.layers[layer_i].mlp.up_proj.weight,
        )
    ...
    if args.vocab_size > model.model.embed_tokens.weight.size(0):
        ...
        pad_weight = torch.zeros(
            [pad_size, model.model.embed_tokens.weight.size(1)]
        ).to(params_dtype)
        out_model["model"]["language_model"]["embedding"]["word_embeddings"][
            "weight"
        ] = torch.cat(
            [model.model.embed_tokens.weight.to(params_dtype), pad_weight], dim=0
        )
        out_model["model"]["language_model"]["output_layer"]["weight"] = torch.cat(
            [model.lm_head.weight.to(params_dtype), pad_weight], dim=0
        )
    else:
        out_model["model"]["language_model"]["embedding"]["word_embeddings"][
            "weight"
        ] = model.model.embed_tokens.weight[: args.vocab_size].to(params_dtype)
        out_model["model"]["language_model"]["output_layer"]["weight"] = (
            model.lm_head.weight[: args.vocab_size].to(params_dtype)
        )

    out_model["model"]["language_model"]["encoder"]["final_layernorm.weight"] = (
        model.model.norm.weight.to(params_dtype)
    )
...
def main():
    ...
    parser.add_argument(
        ...
        "-ref_dir",
        "-r",
        help="folder name of ref model files, mainly to copy the args setting",
        required=True,
    )
    parser.add_argument(
