...
def calc_params_l2_norm(model):
    ...
    max_param = max(param.abs().max() for param in params_data)
    max_param_musa = torch.musa.FloatTensor([float(max_param)])
    ...
    max_param = max_param_musa[0].item()

    return norm_2.item() ** 0.5, max_param
