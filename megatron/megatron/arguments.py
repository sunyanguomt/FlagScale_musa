...
def parse_args(extra_args_provider=None, ignore_unknown_args=False):
    ...
    parser = _add_async_ckpt_args(parser)
...
def validate_args(args, defaults={}):
    ...
    if args.hetero_mode:
        ...
        ), "global_batch_size should be specified when hetero_mode is not None"
        assert (
            args.hetero_current_device_type
        ), "hetero_current_device_type should be specified when hetero_mode is not None"
        assert (
            args.hetero_device_types
        ), "hetero_device_types should be specified when hetero_mode is not None"
        assert len(args.hetero_device_types) == len(
            set(args.hetero_device_types)
    ...
    if args.hetero_mode == "dp":
        ...
        assert (
            args.hetero_pipeline_stages is None
        ), "hetero_pipeline_stages should be None when hetero_mode is dp"
        assert (
            args.micro_batch_size is None
        ...
        args.micro_batch_size = hetero_micro_batch_sizes[
            args.hetero_device_types.index(args.hetero_current_device_type)
        ]

        assert len(args.hetero_micro_batch_sizes) == len(
            args.hetero_device_types
    ...
    if args.hetero_mode == "pp":
        ...
        for split, stages in zip(
            args.hetero_pipeline_stage_splits, args.hetero_pipeline_stages
        ):
            assert split == len(
                stages
        ...
        ), f"sum of hetero_pipeline_stages {sum} should be equal to num_layers {args.num_layers}"
        assert args.pipeline_model_parallel_size == sum(
            args.hetero_pipeline_stage_splits
        ...
        assert len(args.hetero_pipeline_stage_splits) == len(
            args.hetero_device_types
...
def _add_server_args(parser):
    ...
    group.add_argument("--server-port", type=int, default=5060, help="server port.")
    group.add_argument(
        "--model-info", type=str, default="none", help="model infomation"
    )
    return parser
...
def _add_mup_args(parser):
    ...
    group.add_argument(
        "--mup-delta-hidden-size",
        type=int,
        default=128,
        help="Set hidden size for mup delta.",
    )
    group.add_argument(
        "--mup-coord-check-modules",
        nargs="+",
        default=[],
        help="Which modules to report "
...
def _add_customized_device_args(parser):
    ...
    group.add_argument(
        "--device-type", type=str, default=None, help="Specify customized device type"
    )

    return parser


def _add_async_ckpt_args(parser):
    group = parser.add_argument_group(title="heterogeneous training")

    group.add_argument(
        "--enable_async_ckpt", action="store_true", help="if enable async checkpoint"
    )
    group.add_argument(
        "--local_dir", type=str, default=None, help="local storage for checkpoint cache"
    )
    group.add_argument(
        "--mem_dir", type=str, default=None, help="local mem dir for checkpoint cache"
    )
    group.add_argument(
        "--use_file_mmap_ckpt",
        action="store_false",
        help="if enable write checkpoint to localcache",
        dest="disable_file_mmap_ckpt",
    )
