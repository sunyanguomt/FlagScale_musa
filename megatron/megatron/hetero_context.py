...
class HeteroContext:
    ...
    def _build_rank_mapping(self):
        ...
        cur_rank_info = {"rank": rank, "device_type": self._hetero_current_device_type}
        torch.distributed.all_gather_object(all_rank_infos, cur_rank_info)
        ...
        def _compare(rank1, rank2):
            ...
            if self._hetero_device_types and self._hetero_device_types.index(
                device_type1
            ) < self._hetero_device_types.index(device_type2):
                return -1
            elif self._hetero_device_types and self._hetero_device_types.index(
                device_type1
            ) > self._hetero_device_types.index(device_type2):
                return 1
            else:
        ...
        sorted_physical_ranks = sorted(physical_ranks, key=cmp_to_key(_compare))
    ...
    def to_physical_ranks(self, logical_ranks):
        ...
        for logical_rank in logical_ranks:
            physical_ranks.append(self._logical_rank_to_physical_rank[logical_rank])
        return physical_ranks

    def to_logical_ranks(self, physical_ranks):
        logical_ranks = []
        for physical_rank in physical_ranks:
            logical_ranks.append(self._physical_rank_to_logical_rank[physical_rank])
