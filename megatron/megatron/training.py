...
# from elastic_agent.client.event import EventClient, CkptSaveEvent, CkptLoadEvent
...
from megatron.checkpointing import get_checkpoint_name, get_ckeckpoint_dir
...
from megatron import model_studio_monitor
...
def throughput_calculator(args, elapsed_time_per_iter, consumed_tokens_per_iter):
    ...
    peak_tflops = 458e12
    batch_size = consumed_tokens_per_iter / args.seq_length

    # system_throughput = float(consumed_tokens_per_iter) / elapsed_time_per_iter
    # world_size = args.world_size
    # chip_throughput = system_throughput / world_size
    # # For 70B
    # MFU = chip_throughput * 6 * 7e10 / 458e12
    # model_size = os.environ.get("MASS_MODEL_PARAMETER")
    # if model_size == "7b":
    #     MFU = chip_throughput * 6 * 7e9 / 458e12
    # elif model_size == "70b":
    #     MFU = chip_throughput * 6 * 7e10 / 458e12
    # elif model_size == "34b":
    #     MFU = chip_throughput * 6 * 34e9 / 458e12
    # elif model_size == "13b":
    #     MFU = chip_throughput * 6 * 13e9 / 458e12

    # MFU = MFU * (1 + args.seq_length / (6 * args.hidden_size))
    num_hidden_layers = 32
    intermediate_size = 11008
    system_throughput = float(consumed_tokens_per_iter) / elapsed_time_per_iter
    world_size = args.world_size
    chip_throughput = system_throughput / world_size
    flops_per_step = 6 * args.seq_length * batch_size * args.hidden_size * (3 * num_hidden_layers * intermediate_size + 4 * num_hidden_layers * args.hidden_size + args.vocab_size)
    MFU = flops_per_step / (world_size * elapsed_time_per_iter * peak_tflops)
    return MFU * 100, chip_throughput
...
def pretrain(
    ...
    if torch.distributed.get_rank() == 0 and args.load is not None:
        ckpt_iter = os.environ.get("MODEL_STUDIO_CHECKPOINT_ITER", "")
        print_datetime(f"load ckpt iter {ckpt_iter}")
        if ckpt_iter:
            # event = CkptLoadEvent(ckpt_iter=ckpt_iter)
            print_datetime(f"load ckpt event: {ckpt_iter}")
            # EventClient().enqueue(event=CkptLoadEvent(
            #    ckpt_iter = ckpt_iter
            #    ))
        ...
        if args.DDP_impl == "torch":
            ...
            if args.save_param_index_maps_only:
                if (
                    ...
                    for model_module in model:
                        param_name_to_index_maps.append(
                            model_module.param_name_to_index_map
                        )
                    ...
                    index_map_file = os.path.join(
                        index_map_dir, "param_name_to_index_maps.json"
                    )
...
def mup_prepare(
    ...
    unwrapped_based_model = unwrap_model(
        based_model, (torchDDP, LocalDDP, Float16Module)
    )

    args.hidden_size = args.mup_delta_hidden_size
    mup_refresh_args(args)
    delta_model = get_model(model_provider_func, model_type)
    unwrapped_delta_model = unwrap_model(
        delta_model, (torchDDP, LocalDDP, Float16Module)
    )

    os.makedirs(args.mup_save, exist_ok=True)
    for i, (based, delta) in enumerate(
        zip(unwrapped_based_model, unwrapped_delta_model)
    ):
        savefile = os.path.join(args.mup_save, "mup_base_shapes_{}.bsh".format(i))
...
def mup_apply(model):
    ...
    def _init_weights(module):
        ...
        """
        if (
            isinstance(module, tensor_parallel.MuReadoutColumnParallelLinear)
            and args.readout_zero_init
        ):
            module.weight.data.zero_()
        elif isinstance(
            module,
            (tensor_parallel.ColumnParallelLinear, tensor_parallel.RowParallelLinear),
        ):
        ...
        for name, p in module.named_parameters():
            if (
                "self_attention.dense.weight" in name
...
def train_step(
    ...
    if update_successful:
        if args.hetero_mode != "dp":
            ...
            micro_batch_for_all_data_parallel = sum(
                map(
                    lambda x, y: x * y,
            ...
            increment = get_num_microbatches() * micro_batch_for_all_data_parallel
...
def training_log(
    ...
    if iteration % args.log_interval == 0:
        ...
        mfu, chip_throughput = throughput_calculator(
            args, elapsed_time_per_iteration, batch_size * args.seq_length
        )
        log_string += " mfu: {:.4f} | chip_throughput: {:.2f} /s ".format(
            mfu, chip_throughput
        )
        ...
        if torch.distributed.get_rank() == (torch.distributed.get_world_size() - 1):
            ...
            mlflow_metrics["consumed-tokens"] = (
                args.consumed_train_samples * args.seq_length / 1000.0 / 1000 / 1000
            )
            mlflow_metrics["batch-size"] = batch_size
            # for key in loss_dict:
            #     mlflow_metrics[key] = loss_dict[key]
        ...
        for key in total_loss_dict:
            if key not in [advanced_iters_key, skipped_iters_key, nan_iters_key]:
                ...
                if avg > 0.0:
                    ...
                    mlflow_metrics["loss"] = avg
                total_loss_dict[key] = torch.musa.FloatTensor([0.0])
        log_string += " loss scale: {:.1f} |".format(loss_scale)
        if grad_norm is not None:
            # [max_total_norm_before, total_norm, max_total_norm_after, total_norm_after]
            if len(grad_norm) == 2:
                log_string += " max_total_norm_before_clip: {:.3f} |".format(
                    grad_norm[0]
                )
                log_string += " grad norm: {:.3f} |".format(grad_norm[1])
            else:
                log_string += " max_total_norm_before_clip: {:.3f} |".format(
                    grad_norm[0]
                )
                log_string += " grad norm: {:.3f} |".format(grad_norm[1])
                log_string += " max_total_norm_after_clip: {:.3f} |".format(
                    grad_norm[2]
                )
                log_string += " grad_norm_after_clip: {:.3f} |".format(grad_norm[3])
        ...
        if params_norm is not None:
            log_string += " params norm: {:.3f} |".format(params_norm[0])
            log_string += " max-params: {:.3f} |".format(params_norm[1])
            if wandb_writer:
        ...
        ) and os.getenv("MLFLOW_TRACKING_URI", default=None):
...
def save_checkpoint_and_time(iteration, model, optimizer, opt_param_scheduler):
    ...
    if not torch.distributed.is_initialized() or mpu.get_data_parallel_rank() == 0:
        ...
        if not os.path.exists(index_map_dir):
            os.makedirs(index_map_dir, True)
    ...
    if torch.distributed.get_rank() == 0:
        ...
        # EventClient().enqueue(event=CkptSaveEvent(
        #    path=args.save,
        #    step=iteration,
        #    size=calculate_total_size(get_ckeckpoint_dir(args.save, iteration=iteration))
        #    ))
...
def train(
    ...
    while iteration < args.train_iters:
        ...
        if args.mup_coord_check:
            ...
            for m in model:
                for name, module in m.named_modules():
                    if len(coord_check_modules) > 0 and not any(
                        ele in name for ele in coord_check_modules
                    ):
                        continue
                    remove_hooks.append(
        ...
        if args.hetero_mode != "dp":
            ...
            args.consumed_train_samples += (
                get_num_microbatches() * micro_batch_for_all_data_parallel
            )
        ...
        need_save = False
        if args.rampup_batch_size is not None and args.rampup_save_interval is not None:
            rampup_samples = int(args.rampup_batch_size[2])
            if args.consumed_train_samples < rampup_samples:
                if (
                    args.save
                    and args.rampup_save_interval
                    and iteration % args.rampup_save_interval == 0
                ):
                    need_save = True
            else:
                if (
        ...
        if args.mup_coord_check and iteration >= args.mup_coord_check_steps:
            print_datetime(
                "mup coord check early stop at iteration {}".format(iteration)
            )
            break
...
def search_data(train_valid_test_dataset_provider, get_batch):
    ...
    while iteration < search_end:
        ...
        for microbatch_num in range(get_num_microbatches()):
            tokens, labels, loss_mask, attention_mask, position_ids, dataset_idx = (
    ...
    searched_data_file = os.path.join(
        args.data_searching_save,
        "searched_data_{}_to_{}".format(search_start, search_end),
    )
...
def evaluate_and_print_results(
    ...
    for key in total_loss_dict:
        ...
        if wandb_writer:
            ...
            wandb_metrics["{} validation vs samples".format(key)] = (
                args.consumed_train_samples
            )
