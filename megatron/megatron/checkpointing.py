...
def get_ckeckpoint_dir(checkpoints_path, iteration, release=False):
    if release:
        directory = "release"
    else:
...
def read_metadata(tracker_filename):
    ...
    # The iteration is specified
    metastring = os.environ.get("MODEL_STUDIO_CHECKPOINT_ITER", "")
    if not metastring:
        with open(tracker_filename, "r") as f:
            metastring = f.read().strip()

    iteration, release = get_iteration_by_metastring(metastring=metastring)
    # try:
...
def get_iteration_by_metastring(metastring):
    iteration = -1
    release = False
    ...
    try:
        ...
        release = metastring == "release"
        if not release:
            print_rank_0("ERROR: Invalid metadata string {}.".format(metastring))
    finally:
