...
def build_num_microbatches_calculator_hetero(args):
    ...
    def __init__(
        ...
        assert global_batch_size % micro_batch_for_all_data_parallel == 0, (
            ...
            " times data parallel size ({})".format(
                global_batch_size, hetero_micro_batch_sizes, hetero_data_parallel_splits
            )
        )
...
class RampupBatchsizeNumMicroBatches(NumMicroBatchesCalculator):
    ...
    def __init__(
        ...
        assert self.micro_batch_for_all_data_parallel > 0
    ...
    def update(self, consumed_samples, consistency_check):
        ...
        if consistency_check:
            assert (
                self.current_global_batch_size % self.micro_batch_for_all_data_parallel
                == 0
            ), (
                "current global "
                "batch size ({}) is not divisible by the sum of hetero-micro-batch-sizes ({}) times"
                "hetero-data-parallel-splits ({})".format(
                    self.current_global_batch_size,
                    self.hetero_micro_batch_sizes,
