...
class DistributedOptimizer(MixedPrecisionOptimizer):
        ...
        for param, param_world_indexes in param_world_index_map.items():
            ...
            if param_local_end > param_local_start:
                ...
                param_range_map[param] = {
                    ...
                    "name" : model.param_to_name[param]
                }
        ...
        for group_index, group_range in enumerate(opt_group_ranges):
            ...
            for model_param in group_range["params"]:
                ...
                # fp32 params.
                    ...
                    print('model_param.type()', model_param.type(), model_param.size())
    ...
    def __init__(self, optimizer, clip_grad, log_num_zeros_in_grad,
        ...
        # logger.log_to_file('/home/dist/FlagScale/examples/aquila/output/pipe_out' + '/' + launch_time)
        ...
        for i in self.model_gbuf_ranges:
            logger.info(f"i.key, f{i.keys()}")
            logger.info(f"local: {str(i[torch.float16]['local'])}")
            logger.info(f"world: {str(i[torch.float16]['world'])}")
            logger.info(f"world_all: {str(i[torch.float16]['world_all'])}")
            logger.info(f"max_range_size: {str(i[torch.float16]['max_range_size'])}")
            j = i[torch.float16]['param_map']
            for k, v in j.items():
                logger.info(f"parameter name {v['name']}")
                logger.info(f"parameter shape {k.nelement()}, shape is " + ' '.join([str(k.shape[i]) for i in range(len(k.shape))]))
                logger.info(f"gbuf_world: {str(v['gbuf_world'])}")
                logger.info(f"gbuf_local: {str(v['gbuf_local'])}")
                logger.info(f"param: {str(v['param'])}")
    ...
    def reduce_model_grads(self, args, timers):
        ...
        for model in self.models:
            for dtype, gbuf in model._grad_buffers.items():
                # print('gbuf.data.shape', gbuf.data.shape)
                # print('gbuf.data.size', gbuf.data.size())
                gbuf.data /= data_parallel_world_size
                # gbuf.data.div_(data_parallel_world_size)
                # gbuf.data = gbuf.data / data_parallel_world_size
